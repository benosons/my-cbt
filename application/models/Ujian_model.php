...
class Ujian_model extends CI_Model {
    ...
    {
        $this->datatables->select('a.id_ujian, a.token, a.nama_ujian, b.nama_matkul, a.jumlah_soal, CONCAT(a.tgl_mulai, " <br/> (", a.waktu, " Menit)") as waktu, a.jenis, a.kategori_id, c.nama_kategori');
        $this->datatables->from('m_ujian a');
        $this->datatables->join('matkul b', 'a.matkul_id = b.id_matkul');
        $this->datatables->join('kategori c', 'a.kategori_id = c.id_kategori');
        if($id!==null){
    ...
    public function getUjianById($id)
    {
        ...
        $this->db->join('kategori d', 'b.kategori_id=d.id_kategori');
    ...
    {
        $this->db->select('id_dosen, nama_dosen, kategori_id')->from('dosen')->where('nip', $nip);
        return $this->db->get()->row();
    }
    ...
    public function getIdMahasiswa($nim)
    {
        ...
        $this->db->join('kategori d', 'a.kategori_id=d.id_kategori');
    ...
    public function HslUjianById($id, $dt=false)
    {
        ...
        $this->$db->select('d.id, a.nama, b.nama_kelas, c.nama_jurusan, d.jml_benar, d.nilai, e.id_kategori, e.nama_kategori');
        $this->$db->from('mahasiswa a');
        $this->$db->join('kelas b', 'a.kelas_id=b.id_kelas');
        $this->$db->join('jurusan c', 'b.jurusan_id=c.id_jurusan');
        $this->$db->join('h_ujian d', 'a.id_mahasiswa=d.mahasiswa_id');
        $this->$db->join('kategori e', 'a.kategori_id=e.id_kategori');
