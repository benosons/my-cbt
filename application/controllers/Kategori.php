...
class Kategori extends CI_Controller
{
	...
	{
		$rows = count($this->input->post('nama_kategori', true));
		$mode = $this->input->post('mode', true);
		for ($i = 1; $i <= $rows; $i++) {
			$nama_kategori = 'nama_kategori[' . $i . ']';
			...
			if ($this->form_validation->run() === FALSE) {
				...
				if ($mode == 'add') {
					...
					$update[] = array(
						'id_kategori'	=> $this->input->post('id_kategori[' . $i . ']', true),
						'nama_kategori' 	=> $this->input->post($nama_kategori, true)
					);
				}
				$status = TRUE;
		...
		if ($status) {
			if ($mode == 'add') {
				$this->master->create('kategori', $insert, true);
				$data['insert']	= $insert;
			} else if ($mode == 'edit') {
				$this->master->update('kategori', $update, 'id_kategori', null, true);
				$data['update'] = $update;
			}
		} else {
			if (isset($error)) {
	...
	public function delete()
	{
		...
		if (!$chk) {
			...
			if ($this->master->delete('kategori', $chk, 'id_kategori')) {
